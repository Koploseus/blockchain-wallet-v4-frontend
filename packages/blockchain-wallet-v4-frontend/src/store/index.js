...
import { coreMiddleware, coreSagasFactory } from 'blockchain-wallet-v4/src'
import {
  createWalletApi,
  Socket,
  ApiSocket,
...
const configureStore = async () => {
  ...
  const { Connection: MainProcessConnection, middleware: IPCmiddleware } = IPC()
  ...
  const mainProcessConnection = await MainProcessConnection({
    coreSagas,
    input: kernel.multiplexRealm({ tag: `realms` }, window),
    output: kernel.multiplexRealm({ tag: `realms` }, mainProcessWindow),
    outputOrigin: options.domains.mainProcess,
    sagaMiddleware,
    store
  })

  mainProcessConnection.addEventListener(`error`, console.error)
